...
const categoryFilterSkeletons = Array.from({ length: 4 }, (_, index) => index);
const categoryProductSkeletons = Array.from({ length: 8 }, (_, index) => index);
const categoryChipSkeletons = Array.from({ length: 4 }, (_, index) => index);
    ...
    <main aria-busy="true" aria-label={label} className="flex-1">
      <Container className="pt-6">
        ...
        <header className="border-b pt-9 pb-7 sm:flex sm:items-end sm:justify-between sm:gap-8">
          ...
          <Skeleton className="mt-4 h-12 w-full sm:mt-0 sm:w-96" />
        </header>

        <section aria-hidden="true" className="border-b py-4">
          <div className="flex gap-2 overflow-hidden">
            <Skeleton className="h-11 w-30 shrink-0" />
            {categoryChipSkeletons.map((index) => (
              <Skeleton className="h-11 w-28 shrink-0" key={index} />
            ))}
          </div>
        </section>
      </Container>
        ...
        <div className="grid gap-8 pt-8 lg:grid-cols-[13.75rem_minmax(0,1fr)] lg:gap-10 xl:gap-12">
          ...
          >
            ...
            <div className="mt-6 space-y-5">
              {categoryFilterSkeletons.map((index) => (
          ...
          <section aria-label="Produktliste wird geladen" className="min-w-0">
            ...
            >
              {categoryProductSkeletons.map((index) => (
                <CategoryProductSkeleton key={index} />
              ))}
            </div>
